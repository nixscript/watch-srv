...
последнюю строку и заменяем в ней *login* *server.ru*
*22* и *0* на нужные параметры.

    ```
    check "login" "server.ru" 22 0
    ```
    0 - уникальный идентификатор уведомления. Каждый сервер
    должен иметь свой идентификатор, если мониторите несколько
    серверов. К примеру *0*, *1*, *2* ...
